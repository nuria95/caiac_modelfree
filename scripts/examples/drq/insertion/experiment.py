...
from multimexmf.commons.max_entropy_algorithms.max_entropy_drq import MaxEntDrQ
...
def experiment(
    ...
    env = lambda: TimeLimit(InsertionEnv(state_type='vision',
                                         im_size=84,
                                         no_rotation=True,
                                         tactile_shape=(2, 2),
                                         compress_img=True,
    ...
    vec_env = VecFrameStack(make_vec_env(env, n_envs=num_envs, seed=seed), n_stack=4)
    eval_env = VecFrameStack(make_vec_env(env, n_envs=2, seed=seed + 1000), n_stack=4)
    ...
    if alg == 'DrQ':
        ...
        dynnamics_entropy_schedule = lambda x: float(x > 0)
