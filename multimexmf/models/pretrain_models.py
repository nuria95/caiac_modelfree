...
class MultiHeadGaussianEnsemble(nn.Module):
    def __init__(self, input_dim: int, output_dict: Dict, num_heads: int = 5,
        ...
        if self.learn_std:
            output_modules = [[key, nn.Linear(prev_size, 2 * num_heads * value.shape[-1])]
                              for key, value in output_dict.items()]
        else:
            output_modules = [[key, nn.Linear(prev_size, num_heads * value.shape[-1])]
                              for key, value in output_dict.items()]
    ...
    def forward(self, x) -> Dict:
        ...
        for key, value in self.output_dict.items():
            ...
            if self.learn_std:
                mean, log_std = torch.split(pred, 2, dim=-2)
...
if __name__ == '__main__':
    ...
    model = MultiHeadGaussianEnsemble(input_dim=1, output_dict={'y': ys}, features=(256, 256),
                                      optimizer_kwargs={'lr': 1e-3, 'weight_decay': 1e-4}, num_heads=10,
                                      learn_std=learn_std)
    ...
    for j in range(output_dim):
        ...
        plt.fill_between(test_xs.reshape(-1),
                         (y_mean[..., j] - 2 * eps_std[..., j]).reshape(-1),
                         (y_mean[..., j] + 2 * eps_std[..., j]).reshape(-1),
